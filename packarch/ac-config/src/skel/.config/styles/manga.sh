...
cat "$DIR"/styles/manga > "$XDIR"/colors
xrdb ~/.Xresources
...
divers () {
	...
	betterlockscreen -u /usr/share/backgrounds/manga.jpg
	nitrogen --set-zoom-fill --save /usr/share/backgrounds/manga.jpg
	sed -i -e 's/progressbar_color = .*/progressbar_color = "black"/g' $HOME/.ncmpcpp/config
	sed -i -e 's/STYLE=.*/STYLE="manga"/g' $DIR/polybar/launch.sh
