...
cat "$DIR"/styles/archlinux > "$XDIR"/colors
xrdb ~/.Xresources
...
cava () {
	...
	sed -i -e "s/gradient_color_1 =.*/gradient_color_1 = '#59cc33'/" $DIR/cava/config
	sed -i -e "s/gradient_color_2 =.*/gradient_color_2 = '#80cc33'/" $DIR/cava/config
	sed -i -e "s/gradient_color_3 =.*/gradient_color_3 = '#a6cc33'/" $DIR/cava/config
	sed -i -e "s/gradient_color_4 =.*/gradient_color_4 = '#cccc33'/" $DIR/cava/config
	sed -i -e "s/gradient_color_5 =.*/gradient_color_5 = '#cca633'/" $DIR/cava/config
	sed -i -e "s/gradient_color_6 =.*/gradient_color_6 = '#cc8033'/" $DIR/cava/config
	sed -i -e "s/gradient_color_7 =.*/gradient_color_7 = '#cc5933'/" $DIR/cava/config
	sed -i -e "s/gradient_color_8 =.*/gradient_color_8 = '#cc3333'/" $DIR/cava/config
	pkill -USR1 cava
}
...
gtk () {
	...
	xfconf-query -c xsettings -p /Net/IconThemeName -s "Sardi-Flat"
...
divers () {
	...
	betterlockscreen -u /usr/share/backgrounds/archlinux.jpg
	nitrogen --set-zoom-fill --save /usr/share/backgrounds/archlinux.jpg
	sed -i -e 's/progressbar_color = .*/progressbar_color = "black"/g' $HOME/.ncmpcpp/config
	sed -i -e 's/STYLE=.*/STYLE="archlinux"/g' $DIR/polybar/launch.sh
	sed -i -e 's/bspc config bottom_padding.*/bspc config bottom_padding 0/g' $DIR/bspwm/bspwmrc
	sed -i -e 's/bspc config top_padding.*/bspc config top_padding 30; bspc config bottom_padding 0; bspc config -d focused window_gap 10/g' $DIR/bspwm/sxhkd/sxhkdrc
	pkill -USR1 -x sxhkd
}
