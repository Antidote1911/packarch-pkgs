...
cat "$DIR"/styles/packarch > "$XDIR"/colors
xrdb ~/.Xresources
...
xmenu () {
	...
	sed -i -e 's/separator_color=.*/separator_color="'${RED}'",/' $DIR/xmenu/config.h
...
divers () {
	...
	sed -i -e 's/STYLE=.*/STYLE="packarch"/g' $DIR/polybar/launch.sh
